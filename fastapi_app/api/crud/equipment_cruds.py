...
async def create_equipment(new_equipment: EquipmentCreate, session: AsyncSession) -> Equipment:
    equipment = EquipmentCreate(**new_equipment.model_dump())
...
async def update_equipment(equipment_id: int, equipment_data: EquipmentBase, session: AsyncSession) -> Equipment:
    stmt = (
        update(Equipment)
